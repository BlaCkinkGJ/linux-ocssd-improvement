...
#define PBLK_CACHE_BLK_SECTORS (512)
#define PBLK_NR_CACHE_BLK (10)

struct pblk_cache_map_tab {
	int nr_secs_per_blk; /* number of sectors per cache block.
	                      * cache block size = (PBLK_CACHE_BLK_SECTORS * geo->csecs);
												*/
	__le64 *cache_blk[PBLK_NR_CACHE_BLK];
};

struct pblk_g_dir {
	struct pblk_line *line; /* Line where mapping block exists */
	u64 ssecs; /* Sector where mapping block starts */
	atomic_t is_dirty;
	__le64 *cache_blk; /* cache block pointer */
};
...
struct pblk_line_mgmt {
	...
	struct pblk_line *trans_line; /* Current demand trans line */
	struct pblk_line *data_line; /* Current data line */

	struct pblk_line *log_next; /* Next FTL log line */
	struct pblk_line *trans_next; /* Next demand trans line*/
...
struct pblk {
	...
	struct pblk_g_dir *dir; /* Global Translation Directory */
	struct pblk_cache_map_tab *c_tab; /* Cache Mapping Table */
...
static inline struct ppa_addr pblk_ppa32_to_ppa64(struct pblk *pblk, u32 ppa32)
{
	...
	if (ppa32 == -1) {
		...
		if (geo->version == NVM_OCSSD_SPEC_12) {
			...
			ppa64.g.lun =
				(ppa32 & ppaf->lun_mask) >> ppaf->lun_offset;
			ppa64.g.blk =
				(ppa32 & ppaf->blk_mask) >> ppaf->blk_offset;
			ppa64.g.pg = (ppa32 & ppaf->pg_mask) >> ppaf->pg_offset;
			ppa64.g.pl =
				(ppa32 & ppaf->pln_mask) >> ppaf->pln_offset;
			ppa64.g.sec =
				(ppa32 & ppaf->sec_mask) >> ppaf->sec_offset;
		} else {
			struct nvm_addrf *lbaf = &pblk->addrf;

			ppa64.m.grp =
				(ppa32 & lbaf->ch_mask) >> lbaf->ch_offset;
			ppa64.m.pu =
				(ppa32 & lbaf->lun_mask) >> lbaf->lun_offset;
			ppa64.m.chk =
				(ppa32 & lbaf->chk_mask) >> lbaf->chk_offset;
			ppa64.m.sec =
				(ppa32 & lbaf->sec_mask) >> lbaf->sec_offset;
		}
	}
