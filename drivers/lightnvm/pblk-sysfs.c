...
static ssize_t pblk_sysfs_lines(struct pblk *pblk, char *page)
{
	...
	sz += snprintf(page + sz, PAGE_SIZE - sz,
		"lines:d:%d,l:%d,t:%d-f:%d,m:%d/%d,c:%d,b:%d,co:%d(d:%d,l:%d,t:%d)t:%d\n",
					...
					closed_line_cnt,
					bad, cor,
					d_line_cnt, l_line_cnt, t_line_cnt,
					l_mg->nr_lines);
	...
	spin_lock(&l_mg->free_lock);
	if (l_mg->trans_line) {
		cur_sec = l_mg->trans_line->cur_sec;
		msecs = l_mg->trans_line->left_msecs;
		vsc = le32_to_cpu(*l_mg->trans_line->vsc);
		sec_in_line = l_mg->trans_line->sec_in_line;
		meta_weight = bitmap_weight(&l_mg->meta_bitmap,
							PBLK_DATA_LINES);
		map_weight = bitmap_weight(l_mg->trans_line->map_bitmap,
							lm->sec_per_line);
	}
	spin_unlock(&l_mg->free_lock);
	sz += snprintf(page + sz, PAGE_SIZE - sz,
		"trans (%d) cur:%d, left:%d, vsc:%d, s:%d, map:%d/%d (%d)\n",
			...
			atomic_read(&pblk->inflight_io));
	{
		...
		sz += snprintf(page + sz, PAGE_SIZE - sz,
				"id\tline\tpaddr\thot\tbit\tcache\n");
		for(i = 0; i < dir->entry_num; i++) {
			struct pblk_trans_entry *entry = &dir->entry[i];
			int hot = atomic_read(&entry->hot_ratio);
			long int bit_idx = atomic64_read(&entry->bit_idx);
			sz += snprintf(page + sz, PAGE_SIZE - sz,
					"%d\t%d\t%lld\t%d\t%lu\t%p\n",
					i, entry->line->id, entry->paddr, hot, bit_idx, entry->cache_ptr);
